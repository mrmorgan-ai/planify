...
describe('authorize', () => {
  ...
  it('trusts the development identity on a loopback host', async () => {
    ...
    expect(await authorize(request('http://127.0.0.1:8788/api/state'), local, keys)).toBeNull()
    expect(await authorize(request('http://localhost:5173/api/state'), local, keys)).toBeNull()
  })
