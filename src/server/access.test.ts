...
const DEPLOYED = 'https://planify-9ag.pages.dev/api/state'
...
describe('authenticate', () => {
  ...
  it.each([
    ...
    ['a token from another team', () => token(undefined, { issuer: 'https://evil.cloudflareaccess.com' })],
  ...
  it('trusts the development identity on a loopback host', async () => {
    ...
    expect(await authenticate(request('http://127.0.0.1:8788/api/state'), local, keys)).toEqual(dev)
    expect(await authenticate(request('http://localhost:5173/api/state'), local, keys)).toEqual(dev)
  })
