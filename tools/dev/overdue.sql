...
-- dates, so never point it at the real roadmap. It moves the first roadmap, the
-- one local development signs in to.
--
...
WHERE completed_at IS NOT NULL
  AND roadmap_id = 1 AND EXISTS (SELECT 1 FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift');

UPDATE blackouts SET
  from_date = date(from_date, (SELECT value FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift')),
  to_date   = date(to_date,   (SELECT value FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift'))
WHERE roadmap_id = 1 AND EXISTS (SELECT 1 FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift');

UPDATE meta SET value = date(value, (SELECT value FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift'))
WHERE roadmap_id = 1 AND key = 'start_date' AND value <> ''
  AND EXISTS (SELECT 1 FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift');

DELETE FROM meta WHERE roadmap_id = 1 AND key = 'dev_shift';
